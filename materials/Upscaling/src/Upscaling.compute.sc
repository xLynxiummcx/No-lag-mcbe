...
void TAAU(uvec3 GlobalInvocationID) {
    ...
    vec4 currentColor = texelFetch(s_InputFinalColor, intRenderPos, 0);
    ...
    c1 /= 9.0;
    c2 /= 9.0;
    vec3 extent = sqrt(max(vec3(0.0), c2 - c1 * c1));
    float motionWeight = smoothstep(0.0, 1.0, length(motionPixels));
    ...
    posPreviousPixels = clamp(posPreviousPixels, vec2(0.0), DisplayResolution.xy - vec2(1.0));
    vec3 prevColor = bicubicSampleCatmullRom(s_InputTAAHistory, posPreviousPixels, RecipDisplayResolution.xy);
    prevColor = clamp(prevColor, minValidColor, maxValidColor);
